...
class Promo extends CI_Controller {
	...
	function index()
	{
		...
		$this->load->view(akses().'/mitra/promo/promoview',$d);
	...
	{		
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			if($this->mod_produk->promo_add($judul,$deskripsi,$nilai,$mulai,$selesai)==TRUE)
			{
				...
				redirect(base_url(akses().'/mitra/promo'),'refresh',301);
			}else{
				set_header_message('danger','Tambah Kupon Promo','Gagal menambahkan kupon promo');
				redirect(base_url(akses().'/mitra/promo/add'),'refresh',301);
			}
		}else{
			$meta['judul']="Tambah Kupon Promo";
			$this->load->view('tema/header',$meta);
			$this->load->view(akses().'/mitra/promo/promoadd');
			$this->load->view('tema/footer');
		}		
	}
	...
	function edit()
	{
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			$this->load->view(akses().'/mitra/promo/promoedit',$d);
			$this->load->view('tema/footer');
		}		
	}
	...
	function produk()
	{
		...
		$d['promoid']=$id;
		$this->load->view(akses().'/mitra/promo/promoprodukview',$d);
		$this->load->view('tema/footer');
	}
			...
			if($this->m_db->is_bof('promo',$sPromo)==FALSE)
			{
				...
				{					
					if($this->m_db->is_bof('promo_data',$sCek)==TRUE)
					{
						...
						redirect(base_url(akses().'/mitra/promo/produk').'?id='.$promo);
					}else{
				...
				set_header_message('danger','Tambah Produk Promo','Gagal menambah produk promo');
				redirect(base_url(akses().'/mitra/promo'));
			}
		}else{
			set_header_message('danger','Tambah Produk Promo','Gagal menambah produk promo');
			redirect(base_url(akses().'/mitra/promo/produk').'?id='.$promo);
	...
	function gambar()
	{
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			$this->load->view('tema/header',$meta);
			$d['data']=$this->mod_produk->promo_data(array('promo_id'=>$id,'selesai > '=>$tgl));
			$this->load->view(akses().'/mitra/promo/promobanner',$d);
			$this->load->view('tema/footer');
		}
	}
